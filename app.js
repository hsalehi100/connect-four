document.addEventListener('DOMContentLoaded', () => {
        ...
        squares[i].onclick = function() {
            if (squares[index + 7].classList.contains('taken')){
                if(currentPlayer === 1){
                    ...
                    if(displayPlayer.classList.contains('red')){
                        displayPlayer.classList.remove('red');
                        displayPlayer.classList.add('blue');
                    }
                }
                    ...
                    currentPlayer = 1
                    displayPlayer.innerHTML = currentPlayer
                    if(displayPlayer.classList.contains('blue')){
                        displayPlayer.classList.remove('blue');
                        displayPlayer.classList.add('red');
                    }
                }
    ...
    //check board
    function checkBoard() {
        //const showing all wining arrays
        const winningArrays = [
        ...
        //take 4 values and plug them in each squares
        for(let y = 0; y < winningArrays.length; y++){
            ...
            //check if these arrays have the class of player one 
    ...
    //add event litener for each square
